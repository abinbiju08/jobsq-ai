  ...
  return (
    <>
      ...
      <style>{`
        ...
        .ip-card{background:var(--bg3);border:1px solid var(--border);border-radius:14px;padding:1.25rem;cursor:pointer;transition:all .2s;}
      ...
      <div style={{minHeight:'calc(100vh - 52px)',background:'var(--bg)',fontFamily:'Inter,sans-serif'}}>

        {/* Tab bar */}
        <div style={{borderBottom:'1px solid var(--border)',padding:'.75rem 1.5rem',display:'flex',gap:'.5rem',background:'var(--bg2)'}}>
          {[
            ...
            {step==='setup' && (
              <div className="fade-in">
                <div style={{display:'flex',alignItems:'center',gap:'.75rem',marginBottom:'1.5rem'}}>
                  ...
                  <div>
                    <div style={{fontSize:'20px',fontWeight:800,color:'var(--text)'}}>AI mock interview</div>
                    <div style={{fontSize:'12px',color:'var(--text2)'}}>AI plays your interviewer — voice questions, real-time STAR feedback</div>
                  </div>
                </div>
                ...
                <div style={{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'14px',padding:'1.25rem',marginBottom:'1rem'}}>
                  ...
                  <div style={{position:'relative',marginBottom:'.75rem'}}>
                    <input value={roleInput} onChange={e=>{setRoleInput(e.target.value);setRole(e.target.value);setShowRoleDrop(true)}}
                      ...
                      style={{width:'100%',background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'9px',padding:'.6rem .85rem',color:'var(--text)',fontSize:'13px',fontFamily:'Inter,sans-serif',outline:'none',boxSizing:'border-box'}}/>
                    {showRoleDrop && roleInput && filteredRoles.length>0 && (
                      <div style={{position:'absolute',top:'calc(100% + 4px)',left:0,right:0,background:'var(--bg2)',border:'1px solid var(--border2)',borderRadius:'10px',zIndex:100,maxHeight:'180px',overflowY:'auto'}}>
                        {filteredRoles.slice(0,8).map(r=>(
                          <div key={r} onClick={()=>{setRole(r);setRoleInput(r);setShowRoleDrop(false)}}
                            style={{padding:'.5rem .85rem',fontSize:'13px',color:'var(--text2)',cursor:'pointer',transition:'all .15s'}}
                  ...
                  <div style={{display:'flex',gap:'.5rem',marginBottom:'.75rem'}}>
                    <div style={{flex:1}}>
                      <label style={{fontSize:'11px',fontWeight:700,color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.06em',display:'block',marginBottom:'.35rem'}}>Company (optional)</label>
                      <select value={company} onChange={e=>setCompany(e.target.value)} style={{width:'100%',background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'9px',padding:'.55rem .8rem',color:company?'#eef0ff':'#4a5168',fontSize:'13px',fontFamily:'Inter,sans-serif',outline:'none',cursor:'pointer'}}>
                        <option value=''>Select company</option>
                        {COMPANIES.map(c=><option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    <div style={{flex:1}}>
                      <label style={{fontSize:'11px',fontWeight:700,color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.06em',display:'block',marginBottom:'.35rem'}}>Experience level</label>
                      <select value={level} onChange={e=>setLevel(e.target.value)} style={{width:'100%',background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'9px',padding:'.55rem .8rem',color:level?'#eef0ff':'#4a5168',fontSize:'13px',fontFamily:'Inter,sans-serif',outline:'none',cursor:'pointer'}}>
                ...
                {/* Mode picker */}
                <div style={{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'14px',padding:'1.25rem',marginBottom:'1.5rem'}}>
                  <div style={{fontSize:'13px',fontWeight:700,color:'#00e5a0',marginBottom:'.75rem',display:'flex',alignItems:'center',gap:'.4rem'}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'.65rem'}}>
                    {INTERVIEW_MODES.map(m=>(
                      <div key={m.id} className={`ip-card ${mode?.id===m.id?'selected':''}`}
                        ...
                        <div style={{fontSize:'12px',fontWeight:700,color:'var(--text)',marginBottom:'3px'}}>{m.label}</div>
                        <div style={{fontSize:'10px',color:'var(--text2)',lineHeight:1.4}}>{m.desc}</div>
            ...
            {step==='session' && (
              <div className="fade-in">
                ...
                <div style={{display:'flex',alignItems:'center',gap:'1rem',padding:'.85rem 1.25rem',background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'14px',marginBottom:'1.25rem',flexWrap:'wrap'}}>
                  ...
                  <button onClick={()=>setStep('setup')} style={{background:'none',border:'none',color:'var(--text3)',cursor:'pointer',display:'flex',alignItems:'center',fontSize:'12px',gap:'.25rem',fontFamily:'Inter,sans-serif'}}>
                  ...
                  <>
                    ...
                    <div style={{background:'var(--bg3)',border:`1px solid ${mode?.color}30`,borderRadius:'14px',padding:'1.25rem',marginBottom:'1rem'}}>
                      <div style={{display:'flex',alignItems:'flex-start',gap:'.75rem'}}>
                        ...
                        <div style={{flex:1}}>
                          <div style={{fontSize:'11px',color:'var(--text3)',marginBottom:'.4rem',display:'flex',alignItems:'center',gap:'.3rem'}}>
                          ...
                          <div style={{fontSize:'15px',color:'var(--text)',lineHeight:1.6,fontWeight:500}}>
                            {questions[currentQ] || 'Loading question...'}
                          </div>
                        </div>
                        <button onClick={()=>speak(questions[currentQ])} title="Replay question"
                          style={{background:'var(--bg3)',border:'0.5px solid rgba(255,255,255,0.08)',borderRadius:'8px',padding:'.35rem .6rem',color:'var(--text3)',cursor:'pointer',display:'flex',alignItems:'center'}}>
                    ...
                    {!feedback && (
                      <div style={{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'14px',padding:'1.25rem',marginBottom:'1rem'}}>
                        ...
                        <textarea value={currentAnswer} onChange={e=>setCurrentAnswer(e.target.value)}
                          ...
                          style={{width:'100%',background:'rgba(255,255,255,0.03)',border:'0.5px solid rgba(255,255,255,0.08)',borderRadius:'10px',padding:'.75rem .85rem',color:'var(--text)',fontSize:'13px',fontFamily:'Inter,sans-serif',outline:'none',resize:'vertical',minHeight:'100px',lineHeight:1.6,boxSizing:'border-box'}}/>
                      ...
                      <div className="fade-in" style={{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'14px',padding:'1.25rem',marginBottom:'1rem'}}>
                        ...
                        <div style={{display:'flex',alignItems:'center',gap:'1rem',marginBottom:'1rem',padding:'.75rem',background:'var(--bg3)',borderRadius:'10px'}}>
                          ...
                          <div style={{flex:1}}>
                            ...
                            <div style={{display:'flex',gap:'.4rem',marginBottom:'.4rem'}}>
                              ...
                              <span style={{fontSize:'10px',color:'var(--text3)',alignSelf:'center'}}>STAR framework</span>
                            </div>
                          </div>
                        </div>
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'.65rem',marginBottom:'.75rem'}}>
                          <div style={{padding:'.65rem',background:'rgba(0,229,160,0.05)',border:'0.5px solid rgba(0,229,160,0.15)',borderRadius:'9px'}}>
                            ...
                            <div style={{fontSize:'12px',color:'var(--text2)',lineHeight:1.5}}>{feedback.strengths}</div>
                          </div>
                          <div style={{padding:'.65rem',background:'rgba(255,77,109,0.05)',border:'0.5px solid rgba(255,77,109,0.15)',borderRadius:'9px'}}>
                            ...
                            <div style={{fontSize:'12px',color:'var(--text2)',lineHeight:1.5}}>{feedback.weaknesses}</div>
                          </div>
                        </div>
                        <div style={{padding:'.65rem',background:'rgba(124,111,247,0.05)',border:'0.5px solid rgba(124,111,247,0.15)',borderRadius:'9px',marginBottom:'.75rem'}}>
                          ...
                          <div style={{fontSize:'12px',color:'var(--text2)',lineHeight:1.5}}>{feedback.ideal}</div>
                        </div>
                        {feedback.followup && (
                          <div style={{padding:'.65rem',background:'rgba(245,166,35,0.05)',border:'0.5px solid rgba(245,166,35,0.2)',borderRadius:'9px',marginBottom:'.75rem'}}>
                            ...
                            <div style={{fontSize:'12px',color:'var(--text)',lineHeight:1.5,fontStyle:'italic'}}>"{feedback.followup}"</div>
                          </div>
                        )}
            ...
            {step==='report' && report && (
              <div className="fade-in">
                <div style={{textAlign:'center',marginBottom:'1.5rem'}}>
                  <div style={{fontSize:'13px',color:'var(--text2)',marginBottom:'.5rem'}}>Interview complete — {role} {mode?.label}</div>
                ...
                <div style={{display:'flex',alignItems:'center',gap:'1.5rem',padding:'1.25rem',background:'var(--bg3)',border:`1px solid ${scoreColor(report.overall_score)}30`,borderRadius:'16px',marginBottom:'1rem',flexWrap:'wrap'}}>
                  <div style={{textAlign:'center',flexShrink:0}}>
                    <div style={{width:'80px',height:'80px',borderRadius:'50%',border:`4px solid ${scoreColor(report.overall_score)}`,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',margin:'0 auto'}}>
                      ...
                      <div style={{fontSize:'9px',color:'var(--text3)',textTransform:'uppercase'}}>score</div>
                    </div>
                  </div>
                  <div style={{flex:1}}>
                    <div style={{display:'flex',alignItems:'center',gap:'.5rem',marginBottom:'.35rem'}}>
                      <span style={{fontSize:'16px',fontWeight:800,color:'var(--text)'}}>{report.readiness}</span>
                    ...
                    <div style={{fontSize:'13px',color:'var(--text2)',lineHeight:1.6,marginBottom:'.5rem'}}>{report.verdict}</div>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem',marginBottom:'1rem'}}>
                  <div style={{background:'var(--bg3)',border:'0.5px solid rgba(0,229,160,0.2)',borderRadius:'12px',padding:'1rem'}}>
                    ...
                    {report.strengths?.map((s,i)=>(
                      <div key={i} style={{display:'flex',alignItems:'flex-start',gap:'.4rem',marginBottom:'.4rem',fontSize:'12px',color:'var(--text2)'}}>
                        <i className="ti ti-check" style={{fontSize:'13px',color:'#00e5a0',flexShrink:0,marginTop:'1px'}} aria-hidden="true"/>{s}
                      </div>
                    ))}
                  </div>
                  <div style={{background:'var(--bg3)',border:'0.5px solid rgba(255,77,109,0.2)',borderRadius:'12px',padding:'1rem'}}>
                ...
                {report.practice_areas?.length>0 && (
                  <div style={{background:'var(--bg3)',border:'0.5px solid rgba(124,111,247,0.2)',borderRadius:'12px',padding:'1rem',marginBottom:'1rem'}}>
          ...
          <div style={{maxWidth:'860px',margin:'0 auto',padding:'1.5rem 1rem'}}>
            <div className="fade-in">
              <div style={{display:'flex',alignItems:'center',gap:'.75rem',marginBottom:'1.5rem'}}>
                ...
                <div>
                  <div style={{fontSize:'20px',fontWeight:800,color:'var(--text)'}}>Career intelligence</div>
                  <div style={{fontSize:'12px',color:'var(--text2)'}}>AI replacement risk · future outlook · tools to master — for any role</div>
                </div>
              </div>
              ...
              <div style={{display:'flex',gap:'.5rem',marginBottom:'1.5rem'}}>
                <input value={ciRole} onChange={e=>setCiRole(e.target.value)} onKeyDown={e=>e.key==='Enter'&&fetchCareerIntelligence()}
                  ...
                  style={{flex:1,background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'10px',padding:'.65rem 1rem',color:'var(--text)',fontSize:'13px',fontFamily:'Inter,sans-serif',outline:'none'}}/>
                  ...
                  <div style={{display:'flex',flexWrap:'wrap',gap:'.35rem'}}>
                    {['Software Engineer','Data Scientist','Product Manager','UX Designer','Finance Analyst','Marketing Manager','CA / CPA','Doctor / MBBS'].map(r=>(
                      <button key={r} onClick={()=>fetchCareerIntelligence(r)}
                        style={{fontSize:'12px',padding:'.35rem .85rem',borderRadius:'20px',border:'0.5px solid rgba(255,255,255,0.1)',background:'var(--bg3)',color:'var(--text2)',cursor:'pointer',fontFamily:'Inter,sans-serif',transition:'all .15s'}}
              ...
              {ciLoading && (
                <div style={{textAlign:'center',padding:'3rem',color:'var(--text2)'}}>
                  ...
                  <div style={{fontSize:'14px',fontWeight:600,color:'var(--text)',marginBottom:'.35rem'}}>Analysing {ciRole}...</div>
                ...
                <div className="fade-in">
                  ...
                  <div style={{background:'var(--bg3)',border:`1px solid ${ciData.risk_color}30`,borderRadius:'16px',padding:'1.25rem',marginBottom:'1rem'}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'2rem',flexWrap:'wrap'}}>
                      <div style={{textAlign:'center',flexShrink:0}}>
                        ...
                        <div style={{fontSize:'10px',color:'var(--text3)',marginTop:'2px'}}>{ciData.risk_timeline}</div>
                      </div>
                      <div style={{flex:1,display:'grid',gridTemplateColumns:'1fr 1fr',gap:'.65rem'}}>
                        <div style={{padding:'.65rem',background:'rgba(255,77,109,0.05)',border:'0.5px solid rgba(255,77,109,0.15)',borderRadius:'9px'}}>
                          ...
                          {ciData.what_ai_takes?.map((t,i)=><div key={i} style={{fontSize:'11px',color:'var(--text2)',marginBottom:'2px',display:'flex',alignItems:'flex-start',gap:'.3rem'}}><i className="ti ti-minus" style={{fontSize:'11px',color:'#ff4d6d',flexShrink:0,marginTop:'1px'}} aria-hidden="true"/>{t}</div>)}
                        </div>
                        <div style={{padding:'.65rem',background:'rgba(0,229,160,0.05)',border:'0.5px solid rgba(0,229,160,0.15)',borderRadius:'9px'}}>
                          ...
                          {ciData.what_stays_human?.map((t,i)=><div key={i} style={{fontSize:'11px',color:'var(--text2)',marginBottom:'2px',display:'flex',alignItems:'flex-start',gap:'.3rem'}}><i className="ti ti-check" style={{fontSize:'11px',color:'#00e5a0',flexShrink:0,marginTop:'1px'}} aria-hidden="true"/>{t}</div>)}
                        </div>
                      </div>
                    </div>
                    {ciData.safer_pivot_roles?.length>0 && (
                      <div style={{marginTop:'.85rem',paddingTop:'.85rem',borderTop:'0.5px solid rgba(255,255,255,0.06)'}}>
                        <div style={{fontSize:'10px',color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'.4rem'}}>Safer pivot roles</div>
                        <div style={{display:'flex',flexWrap:'wrap',gap:'.35rem'}}>
                  ...
                  <div style={{background:'var(--bg3)',border:'0.5px solid rgba(59,130,246,0.2)',borderRadius:'16px',padding:'1.25rem',marginBottom:'1rem'}}>
                      ...
                      {[['2025',ciData.future_outlook?.demand_2025],['2027',ciData.future_outlook?.demand_2027],['2030',ciData.future_outlook?.demand_2030]].map(([yr,val])=>{
                        ...
                        return (
                          <div key={yr} style={{flex:1,padding:'.75rem',background:`${c}0e`,border:`0.5px solid ${c}30`,borderRadius:'10px',textAlign:'center'}}>
                            <div style={{fontSize:'11px',color:'var(--text3)',marginBottom:'.25rem'}}>{yr}</div>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'.65rem'}}>
                      <div>
                        <div style={{fontSize:'10px',color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'.4rem'}}>Salary trend</div>
                      ...
                      <div>
                        <div style={{fontSize:'10px',color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'.4rem'}}>Top industries hiring</div>
                    ...
                    {ciData.future_outlook?.emerging_subroles?.length>0 && (
                      <div style={{marginTop:'.85rem',paddingTop:'.85rem',borderTop:'0.5px solid rgba(255,255,255,0.06)'}}>
                        <div style={{fontSize:'10px',color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'.4rem'}}>Emerging sub-roles</div>
                        <div style={{display:'flex',flexWrap:'wrap',gap:'.35rem'}}>
                          ...
                          <div key={i} className="tool-card">
                            <div style={{display:'flex',alignItems:'flex-start',gap:'.75rem'}}>
                              ...
                              <div style={{flex:1}}>
                                ...
                                <div style={{fontSize:'12px',color:'var(--text2)',lineHeight:1.4}}>{tool.purpose}</div>
                              </div>
                            </div>
                          </div>
                        )
                  ...
                  <div style={{background:'var(--bg3)',border:'0.5px solid rgba(255,255,255,0.07)',borderRadius:'16px',padding:'1.25rem',marginBottom:'1.25rem'}}>
                    <div style={{display:'flex',alignItems:'flex-start',gap:'.5rem',marginBottom:'.85rem',padding:'.75rem',background:'rgba(124,111,247,0.06)',border:'0.5px solid rgba(124,111,247,0.2)',borderRadius:'10px'}}>
                      ...
                      <div style={{fontSize:'13px',color:'var(--text2)',lineHeight:1.6}}>{ciData.verdict}</div>
                    </div>
                    {ciData.survival_tips?.length>0 && (
                      <>
                        ...
                        {ciData.survival_tips.map((t,i)=>(
                          <div key={i} style={{display:'flex',alignItems:'flex-start',gap:'.5rem',marginBottom:'.4rem',fontSize:'12px',color:'var(--text2)'}}>
                  ...
                  <button onClick={()=>{setCiData(null);setCiRole('');setCiError('')}}
                    style={{display:'flex',alignItems:'center',gap:'.4rem',padding:'.55rem 1rem',background:'none',border:'0.5px solid rgba(255,255,255,0.1)',borderRadius:'9px',color:'var(--text2)',fontSize:'12px',cursor:'pointer',fontFamily:'Inter,sans-serif'}}>
